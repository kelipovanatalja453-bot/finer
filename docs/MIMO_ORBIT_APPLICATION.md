...
目前项目已实现 V0/V1 语义基础：V0 将多源内容标准化为 ContentEnvelope/ContentBlock；V0.5 加入 QualityCard、TemporalAnchor、EntityAnchor、EvidenceSpan；V1 抽取 NormalizedInvestmentIntent，区分“看好”和“加仓”等不同投资语义。项目也采用多 Agent 工作流拆分架构规划、schema 合约、质量门控、fixture 构建、图片策略验证和独立验收。公开仓库中已包含架构文档、执行报告、猫大人聊天/图片策略样本、V0/V1 测试与验证结果。

MiMo Token 将主要用于 OCR/图片策略理解、长聊天清洗、音频转录稿切分、相对时间解析、跨文档观点串联、V1 intent 抽取和 SFT/DPO 数据生成。目标是建立一套可复现的流程，用证据链和模拟跟单收益评估 KOL，而不是依赖主观印象。

## Core Pain Points Solved

1. **Noisy multimodal KOL content**

   KOLs do not publish in a clean research-report format. Inputs include screenshots, tables, charts, long chats, OCR artifacts, PDFs, and transcribed livestreams. Finer normalizes those inputs into a common V0 schema.
...
- V1 intent extraction with evidence spans.
...
| Stage | MiMo Usage |
|---|---|
| V0 cleaning | Normalize OCR, chat logs, PDFs, and transcripts into clean blocks |
| V0.5 anchoring | Extract time references, entities, evidence spans, and ambiguity flags |
| V1 intent extraction | Generate structured `NormalizedInvestmentIntent` with confidence and rationale |
| Policy reasoning | Infer KOL style, risk preference, and personalized mapping assumptions |
| Review assistance | Explain why an intent/action was produced and support human correction |
| Training loop | Generate SFT/DPO data from corrected examples and backtest feedback |

## Evidence in the Repository

- `docs/ARCHITECTURE.md`: target architecture and current implementation status.
- `docs/architecture-alignment-plan.md`: V0/V1/V2/V3/V4/V5 architecture alignment.
- `docs/agent-execution-plan.md`: multi-agent implementation and verification plan.
- `docs/v0-v1-schema-contract-validation-report.md`: validation report for schema contracts and fixtures.
- `src/finer/schemas/content_envelope.py`: V0 content envelope and content block schema.
- `src/finer/schemas/investment_intent.py`: V1 normalized investment intent schema.
- `src/finer/parsing/content_standardizer.py`: rule-based V0 standardizer.
- `src/finer/extraction/intent_extractor.py`: minimal V1 intent extractor.
- `tests/fixtures/kol/`: KOL fixture cases for chat/document and image-strategy samples.
- `tests/test_cat_lord_image_v0_pipeline.py`: image strategy V0 pipeline validation.
...
The skipped tests are async tests that require a dedicated pytest async runtime configuration. They are tracked as an engineering follow-up and are not related to the V0/V1 schema and fixture contract.

## Next Milestones

1. Integrate MiMo into V0 cleaning and V1 intent extraction as a selectable model backend.
2. Add long-context KOL persona / policy generation for 200-1000 historical posts per KOL.
3. Build V2 policy mapping from intent to executable `TradeAction`.
