...
```
F0 INTAKE → F1 STANDARDIZE → F2 ANCHOR → F3 INTENT → F4 POLICY → F5 EXECUTE → F6 REVIEW → F7 TIMELINE → F8 BACKTEST
                                                                                         ↑ F+ Training Loop ↑
```
...
| F5 | Execute | TradeAction | partial |
...
5. **F5 canonical TradeAction MUST include intent_id, policy_id, evidence_span_ids。**
